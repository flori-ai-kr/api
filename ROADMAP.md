...
| SPEC-SERVER-022 | TODO | 018 | **(E2) RestDocs 검증 API 문서 + 커버리지 80%**: onetime 패턴 이식 — ePages `restdocs-api-spec` 0.19.5 + `spring-restdocs-mockmvc`로 기존 통합테스트에서 OpenAPI3 생성 → springdoc 뷰어로 서빙(`api-docs.enabled=false`, `swagger-ui.url=정적스펙`), 컨트롤러 문서 어노테이션 제거. JaCoCo line 80% 게이트. 브랜치 `feature/SPEC-SERVER-022-restdocs`→dev PR. 명세 `.moai/specs/SPEC-SERVER-022/spec.md` |
