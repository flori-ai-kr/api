...
├── auth/                  # 소셜 로그인/가입(register/complete에 phoneNumber 필수 — PII, 응답 비노출; 가입 동의 수집 — termsAgreed·privacyAgreed 필수/@AssertTrue, marketingAgreed 선택 → user_consents 저장·응답 비노출·입증 근거), JWT 발급·refresh rotation, registerToken
...
├── ai/                    # AI 게이트웨이 (/ai/**) — web↔ai-server(FastAPI) 중개 + 모든 AI 호출 DB 로깅. 채팅/proactive/OCR예약/confirm. **마케팅 콘텐츠 CRUD**(블로그 초안 생성·목록·상세·수정·삭제·톤 프로필, `/ai/marketing/**`). ai-server는 내부망 stateless
