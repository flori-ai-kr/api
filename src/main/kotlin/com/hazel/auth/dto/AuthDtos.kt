...
data class SignupRequest(
    ...
    @field:Schema(description = "로그인 이메일", example = "florist@hazel.kr")
    val email: String,
    @field:NotBlank(message = "비밀번호는 필수입니다")
    @field:Size(min = 8, max = 72, message = "비밀번호는 8자 이상이어야 합니다")
    @field:Schema(description = "비밀번호(8~72자). BCrypt로 저장.", example = "hazel1234", minLength = 8, maxLength = 72)
    val password: String,
...
data class LoginRequest(
    ...
    val email: String,
    @field:NotBlank(message = "비밀번호는 필수입니다")
    @field:Schema(description = "비밀번호", example = "hazel1234")
    val password: String,
)
