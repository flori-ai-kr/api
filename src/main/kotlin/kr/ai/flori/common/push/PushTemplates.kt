...
object PushTemplates {
    ...
    fun communityNotice(
        ...
        PushContent(
            title = "새 공지사항",
            body = title,
            link = PushLinks.community(postId),
        )
    ...
    fun auctionScrapUpdate(pumNames: List<String>): PushContent {
        ...
        return PushContent(title = "스크랩한 경매 시세 업데이트", body = body, link = PushLinks.insights())
    }
    ...
    fun grantNew(
        ...
        return PushContent(title = "새 지원사업", body = body, link = PushLinks.insights())
    }
    ...
    fun grantDeadline(
        ...
        if (daysLeft <= 0) {
            PushContent(title = "지원사업 오늘 마감", body = "$title 마감이 오늘입니다", link = PushLinks.insights())
        } else {
