...
## 목표
모든 도메인 리포지토리의 쿼리 메서드가 `user_id`로 테넌트 격리되는지 **자동 검증하는 회귀 가드 테스트**를 추가한다. 새 메서드가 격리를 빠뜨리면 테스트가 실패하도록 만들어, flori의 1순위 HARD 원칙("`user_id` 필터 누락 = 데이터 유출")을 코드 레벨 가드레일로 정착시킨다.

## 배경
- flori은 RLS가 없어 **애플리케이션이 유일한 방어선**(CLAUDE.md 보안 1순위).
- RF-001 audit에서 격리 누락 0건을 **수동** 확인했으나, 미래 변경으로 다시 깨질 수 있음 → 영구 자동 가드 필요.
- 출처 패턴: `onetime/backend`의 `SecurityAnnotationTest`(모든 컨트롤러의 권한 어노테이션 누락을 클래스패스 스캔으로 검출)를 flori의 격리 모델에 맞게 이식.

## 범위
- `src/test/kotlin/com/flori/common/tenant/TenantIsolationGuardTest.kt` 신규.
- 리플렉션으로 `kr.ai.flori.*` 리포지토리 인터페이스의 **직접 선언 메서드**(상속된 JpaRepository CRUD 제외)를 전수 검사.
...
## 인수기준
- [ ] 모든 `kr.ai.flori` 리포지토리 선언 메서드가 격리되거나 화이트리스트에 있음 → 그린
