...
Flori Server의 빈 Spring Boot(Kotlin) 프로젝트를 부팅 가능한 상태로 세운다.
...
- **패키지 구조**: `kr.ai.flori` 루트 + `kr.ai.flori.common`(config/security/tenant/error/storage/push 자리). 도메인 패키지는 후속 SPEC에서 추가.
- **부팅**: `FloriServerApplication` 메인 + `application.yml`(모든 외부 값은 `${ENV}` 참조).
...
3. `GET /health` 가 200과 `status: "UP"`, `service: "flori-ai-server"` 를 반환한다(`@WebMvcTest` 검증).
4. `application.yml` 에 평문 시크릿이 없다(모든 외부 값은 `${ENV}` 참조).
5. 패키지 구조가 `kr.ai.flori` / `kr.ai.flori.common` 으로 잡혀 있어 후속 도메인 패키지를 추가만 하면 된다.
