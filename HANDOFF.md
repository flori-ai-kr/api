...
- 🧹 **데드코드 정리 + OpenAPI 스펙 동기화 완료** (2026-05-27, 커밋 `78c7dad`, dev): code-reviewer 진단 기반 애드혹 정리. ① 미사용 `InstagramAccountRepository#findByUsername` 제거 + `TenantIsolationGuardTest` 화이트리스트 항목 동반 제거. ② 미사용 `SecurityConfig.authenticationManager` 빈 제거(JWT 필터 기반이라 주입처 없음). ③ `/subscription/premium-example` **데모 엔드포인트 제거**(SPEC-SERVER-022 follow-up ② 해소) — `@RequiresSubscription` 게이팅 검증은 운영코드에 더미를 두지 않도록 **테스트 전용 컨트롤러(`/__test__/gated`, @TestConfiguration @Import)** 기반 통합테스트(토큰없음 401 / 미구독 403 / 구독후 200)로 대체(mock 라이브러리 미사용 컨벤션 유지 — 처음 Mockito 단위테스트로 작성됐다가 컨벤션 위반으로 교체). ④ **preferences RestDocs 신설**(`UserPreferenceDocsTest`) — openapi3 재생성 시 DocsTest 부재로 `/settings/preferences`·`/preferences/bottom-nav`가 스펙에서 누락되던 회귀 복구. ⑤ 부수효과로 이전 커밋이 빠뜨린 stale 스펙(`/deposits/*`·`/settings/card-companies/*`, 코드엔 이미 없던 경로) 정리. `./gradlew clean build --rerun-tasks` 294테스트 통과·ktlint/detekt/JaCoCo 80% 통과. **남은 SPEC-022 follow-up**: ①(완료, JWT bearer Authorize), ②(완료, 본 작업), ③ scrap-post-list 항목 필드 문서화 + photo-cards upload-targets presign fake 빈 — 미해소.
- ✅ **SPEC-RN-015 (카카오 OAuth 소셜 로그인) 완료** (2026-05-26): `POST /auth/oauth/kakao`(KakaoOAuthRequest{code, redirectUri} → TokenResponse) 추가. `kr.ai.flori.auth.oauth` 패키지 신설 — `KakaoOAuthClient` 인터페이스(테스트 스텁 가능) + `KakaoOAuthClientImpl`(RestClient, kauth.kakao.com 토큰교환 + kapi.kakao.com 프로필조회). 신규 사용자면 User(provider=KAKAO, providerId) INSERT + 기본설정 시드, 기존이면 findByProviderAndProviderId로 재사용. 동시 첫 로그인 경쟁은 DataIntegrityViolationException 캐치 후 재조회(멱등). 카카오 4xx/5xx는 AppException(INVALID_TOKEN)으로 변환(원인 체이닝). **마이그레이션** `V6__add_oauth_provider.sql`: password_hash·email DROP NOT NULL, provider(DEFAULT 'LOCAL' NOT NULL)/provider_id 컬럼 추가, partial unique index `uq_users_provider_identity`. **외부 의존성**: kauth.kakao.com, kapi.kakao.com. **설정** `KAKAO_REST_API_KEY`·`KAKAO_CLIENT_SECRET` 환경변수(application.yml에 `${ENV}` 참조). **테스트**: `KakaoOAuthClientImplTest`(MockRestServiceServer — 정상 흐름/4xx 변환), `AuthOAuthDocsTest`(StubKakaoClient, 신규생성+재로그인 경로 커버, RestDocs 문서화).
