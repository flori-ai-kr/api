...
> `waitlist/error/WaitlistErrorCode`. 출시 전 선착순 100명 공개 모집. 인증 불필요. 식별자: 이메일(trim+소문자 정규화 후 UNIQUE 비교).

| 코드 | 의미 | HTTP | 발생 지점 |
|------|------|------|-----------|
| `E-WL-001` | ALREADY_REGISTERED (중복 등록) | 409 | 이미 등록된 이메일(정규화 후 UNIQUE 비교)로 재등록 시도 |
| `E-WL-002` | CLOSED (모집 마감) | 409 | 등록 시도 시점에 등록 수가 정원(100명) 이상 |

## E-IV-* (인터뷰 모집)

> `interview/error/InterviewErrorCode`. 유저 인터뷰 참여자 공개 모집. 인증 불필요. 식별자: 전화번호(숫자만 정규화 후 UNIQUE 비교).
