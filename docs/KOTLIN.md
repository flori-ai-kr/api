...
`data class`의 자동 `equals`/`hashCode`는 모든 필드를 쓰는데, JPA의 지연 로딩 프록시·미할당 ID와 충돌해 미묘한 버그를 만든다. 그래서 엔티티는 **일반 `class` + `var` 필드**로 둔다. ([`Sale.kt`](../src/main/kotlin/com/flori/sales/entity/Sale.kt))
