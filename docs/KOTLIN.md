...
fun get(id: UUID): SaleResponse   // 반환 타입도 뒤
```
...
```kotlin
fun get(id: UUID): SaleResponse = saleService.get(id)
...
`data class`의 자동 `equals`/`hashCode`는 모든 필드를 쓰는데, JPA의 지연 로딩 프록시·미할당 ID와 충돌해 미묘한 버그를 만든다. 그래서 엔티티는 **일반 `class` + `var` 필드**로 둔다. ([`Sale.kt`](../src/main/kotlin/com/hazel/sales/entity/Sale.kt))

```kotlin
@Entity
@Table(name = "sales")
class Sale(                                       // data class 아님
    @Column(name = "user_id", nullable = false) var userId: UUID,  // var (영속 상태 변경)
    /* ... */
) {
    @Id
    @GeneratedValue(strategy = GenerationType.UUID)
    var id: UUID? = null                          // 저장 전엔 null → nullable
}
```
- DTO는 `data class`(값), 엔티티는 `class`(영속 객체) — 역할이 다르다.
- `id`가 `UUID? = null`인 이유: `save()` 전엔 ID가 없으므로. 그래서 응답 변환 때 `requireNotNull(sale.id)`로 좁힌다.
