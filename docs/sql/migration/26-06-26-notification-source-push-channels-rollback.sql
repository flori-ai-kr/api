-- 26-06-26-notification-source-push-channels.sql 롤백.
-- source CHECK를 직전 값('web','cron','system','alimtalk')으로 되돌린다.
-- 주의: 되돌리기 전 source IN ('web_push','fcm') 행이 있으면 제약 추가가 실패하므로 사전 정리 필요.
START TRANSACTION;
