...
**0. (스키마가 새로 필요하면) DDL 추가**
`docs/sql/all-tables-ddl.sql`에 `CREATE TABLE coupons ...`를 추가하고, 운영/로컬 DB에 수동 적용한다(DDL 직접 관리, Flyway 미사용). PK는 `id bigint generated by default as identity`, `user_id bigint not null references users(id) on delete cascade` 컬럼 필수. 테스트는 `spring.sql.init`가 이 DDL을 임베디드 PG에 적용하므로 별도 작업 불필요.
