...
- 커뮤니티 검색(CommunityService.listPosts)의 LIKE 패턴 — 같은 이스케이프 누락이 있으나 공개 게시글 대상이라 유출 위험 없음, 기능 오작동 수준 (보안 리뷰 P3). 다음 커뮤니티 작업 때 sales/expenses와 동일하게 수정
- AdminUserService 페이지네이션 — raw SQL LIMIT/OFFSET 직빌딩이라 Pageable 기반 Paging 헬퍼 미적용(보정 규칙은 동일)
